import { motion } from 'framer-motion';

const SvgPhone = () => (
  <svg viewBox="0 0 24 24" fill="none" strokeWidth="1.8" stroke="currentColor" style={{ width: 14, height: 14 }}>
    <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.82 9.8 19.79 19.79 0 01.75 1.17 2 2 0 012.73 0h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.18 6.18l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"/>
  </svg>
);

export default function SosButton() {
  return (
    <motion.a
      href="tel:025578729"
      className="sos-btn"
      initial={{ scale: 0, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      transition={{ type: 'spring', stiffness: 200, damping: 14, delay: 2 }}
      aria-label="Emergency dental care — call now"
    >
      <div className="sos-ring"><div className="sos-ring-inner" /></div>
      <SvgPhone />
      <span>Emergency SOS</span>
    </motion.a>
  );
}
